    ...
    if ($wp_filesystem -> is_file($folder . 'slider.html') &&
        ...
        if (count($images)){
            ...
            if ($id){
                ...
                if (file_exists($dest . 'script.js')) file_put_contents($dest . 'script.js', 
                        // %URL%/images/
                        str_replace('%URL%', WOWSLIDER_PLUGIN_URL . "sliders/$id/", 
                            str_replace('%ID%', $id, file_get_contents($dest . 'script.js'))
                        )
                );
                return true;
...
function wowslider_import($zip_file = false, $update = 0, $delete = true){
    ...
    $install = $path . 'wow-slider-wordpress-image-slider-plugin/install/';
    if ($delete) $wp_filesystem -> delete($zip_file);
    if (is_object($unzip)) $status = __('Wrong .zip file.', 'wowslider');
    else $status = wowslider_add((is_dir($install) ? $install : $path . 'import/'), $update);
...
function wowslider_get($q){
    ...
    if (is_integer($q)){
        ...
        if ($wpdb -> get_var('SELECT ID FROM ' . $wpdb -> prefix . 'wowslider WHERE ID = ' . $id . ($only_public ? ' AND slider_public = 1' : '') . ' LIMIT 1;')){
            ...
            if (file_exists(WOWSLIDER_PLUGIN_PATH . 'sliders/' . $id . '/script.js')) $html .= "<script type='text/javascript' src='" . WOWSLIDER_PLUGIN_URL . "sliders/$id/script.js'></script>\n\n";
            return $html;
        }
        return '';
