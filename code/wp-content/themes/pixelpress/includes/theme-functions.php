...
if (!function_exists( 'woo_display_breadcrumbs')) {
	function woo_display_breadcrumbs() {
		global $woo_options;
		if ( !is_home() && !is_singular( 'post' ) && !is_post_type_archive( 'post' ) && !is_page_template('template-blog.php') && !is_singular( 'portfolio' ) & !is_post_type_archive( 'portfolio' ) & !is_page_template('template-portfolio.php') ) {
...
<aside class="post-meta">
	<ul>
		...
		<li class="post-category">
			<span class="small"><?php _e( 'Categories:', 'woothemes' ) ?> </span>
			<?php the_category( ', ') ?>
		</li>
...
?>
	...
	<?php if ( isset($woo_options['woo_contact_subscribe_and_connect']) && $woo_options['woo_contact_subscribe_and_connect'] == 'true' && is_page_template('template-contact.php') ) { $settings[ 'connect' ] = 'true'; } ?>
...
if ( ! function_exists( 'woo_archive_description' ) ) {
	function woo_archive_description ( $echo = true ) {
		...
		$description = '';
		if ( isset($term_obj->term_id) ) { $description = term_description( $term_obj->term_id, $term_obj->taxonomy ); }
...
function woo_maps_contact_output($args){
    ...
    <?php } ?>
    <script src="<?php echo get_template_directory_uri(); ?>/includes/js/markers.js" type="text/javascript"></script>
...
function woo_add_googlefonts () {
    global $google_fonts;
    $google_fonts[] = array( 'name' => 'Montserrat');
}

/*-----------------------------------------------------------------------------------*/
/* Add Section Title To Blog Sections */
/*-----------------------------------------------------------------------------------*/

add_action('woo_main_before', 'woo_add_section_title', 9);
function woo_add_section_title() {

	$settings = array(
					'blog_section_title' => 'true',
					'blog_section_title_text' => 'Blog',
					'blog_section_message' => 'This is where your latest blog posts will show up. You can change this text in your Theme Options panel.',
					);
					
	$settings = woo_get_dynamic_values( $settings );

	if (is_singular( 'post' ) || is_post_type_archive( 'post' ) || is_page_template('template-blog.php')) {

?>
	
	<?php if ( $settings['blog_section_title'] == 'true' ): ?>
	<section class="section-title blog" class="col-full">
	
		<h1><?php echo $settings['blog_section_title_text']; ?></h1>
		<?php if ( $settings['blog_section_message'] != '' ) { ?><p><span><?php echo $settings['blog_section_message']; ?></span></p><?php } ?>
 ...
 * @return void
 */
function woodojo_tabs_latest ( $limit, $image_dimension ) {
	global $post;
	$html = '';
	...
	foreach( $latest as $post ) {
		...
		if ( $image_dimension > 0 ) {
			if ( WooDojo_Widget_Tabs::get_image( $image_dimension, $post ) ) {
				$html .= '<div class="rounded-image">';
				$html .= WooDojo_Widget_Tabs::get_image( $image_dimension, $post );
				$html .= '</div>';
			}
		}
	...
	return $html;
} // End woodojo_tabs_latest()

/**
 * woodojo_tabs_popular function.
 * 
 * @access public
 * @since 1.0.0
 * @param int $limit
 ...
 * @return void
 */
function woodojo_tabs_popular ( $limit, $image_dimension ) {
	global $post;
	$html = '';
	...
	return $html;
} // End woodojo_tabs_popular()

/**
 * woodojo_tabs_comments function.
 * 
 * @access public
 * @since 1.0.0
 * @param int $limit
 ...
 * @return void
 */
function woodojo_tabs_comments ( $limit, $image_dimension ) {
	global $wpdb;
	$html = '';
	...
	if ( $comments ) {
		...
		foreach( $comments as $c ) {
			$html .= '<li>' . "\n";
			if ( $image_dimension > 0 ) {
				if ( get_avatar( $c, $image_dimension ) ) {
					$html .= '<div class="rounded-image">';
