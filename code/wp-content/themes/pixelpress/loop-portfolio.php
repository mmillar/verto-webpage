...
if ( have_posts() ) : $count = 0; ?>
	<header class="<?php echo $filtering_css_class; ?> section-title portfolio">
		...
		<?php
			if ( $screen_type == 'page' ) { ?><?php the_title(); ?><?php }
			elseif ( $screen_type == 'post-type-archive' ) { echo apply_filters( 'woo_portfolio_type_archive_title', __( 'Portfolio', 'woothemes' ) ) ; }
			elseif ( $screen_type == 'taxonomy' ) { echo $archive_title; }
		?>				    		
		
		</h1>
	
		<?php if ($woo_options['woo_portfolio_section_message'] && $woo_options['woo_portfolio_section_message'] != ''): ?>
			<p><span><?php echo esc_attr($woo_options['woo_portfolio_section_message']); ?></span></p>
		<?php endif; ?>
